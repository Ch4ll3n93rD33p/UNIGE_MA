...
func testOuterMost() {
  let t1: Nat = .add(.add(.var("x"), .zero), .zero)
  let t2: Nat = .add(.add(.var("x"), .zero), .add(.zero,.zero))

  XCTAssertEqual(Strategy.eval(t: t1, s: .outermost(.axiom)) as? Nat, .var("x"))
  XCTAssertEqual(Strategy.eval(t: t2, s: .outermost(.axiom)) as? Nat, Nat.add(.var("x"), .zero))
}
